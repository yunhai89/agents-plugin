...
| 🎭 表情包 | LLM 自主在回复内嵌表情包（图片模式内嵌进图 / 文本混排，频率受控） | 🧪 需 `#表情包安装` |
...
表情源：[bangbang93HUB](https://github.com/Mxmilu666/bangbang93HUB)（插件不内置图，用户指令克隆下载到插件 `resources/stickers/`）。
...
| 指令 | 说明 |
| --- | --- |
| `#表情包安装` | 浅克隆仓库（**自动测速选最快 GitHub 代理**，NSFW 目录默认排除）→ 同步 → 自动开启 |
...
sticker:
  ...
  excludeDirs: ["8Mi_Yile", "0-Dress"]  # NSFW 默认排除
  githubProxies: []        # 克隆加速代理前缀数组（追加在内置代理之上，安装时测速选最快）
```

> ⚠️ **合规**：bangbang93HUB 仓库**可能含 NSFW 内容**，已默认排除 `8Mi_Yile` / `0-Dress` 目录并内置关键词过滤；发送侧只发本地 `images/` 内物理存在的文件（黑名单目录根本不会被复制进来）。表情仅供娱乐，版权归属 bangbang93HUB 及其贡献者。
