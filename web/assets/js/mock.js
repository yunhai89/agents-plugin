...
(function () {
  ...
  /* ---------- §1 配置(敏感字段已是 MaskedValue: {configured, preview}) ---------- */
  const config = {
    trigger: 'both',
    triggerCommand: '#ai',
    ...
    protocol: 'openai',
    preset: 'deepseek',
    baseURL: { configured: true, preview: 'https://api.deepseek.com' },
    apiKey: { configured: true, preview: 'sk-****a3f9' },
    model: 'deepseek-chat',
    utilityModel: 'deepseek-chat',
    fallbackModels: [
      { model: 'gpt-4o-mini', baseURL: { configured: true, preview: 'https://api.openai.com' }, apiKey: { configured: true, preview: 'sk-****7c21' }, protocol: 'openai' },
      { model: 'claude-3-5-haiku-20241022', baseURL: { configured: true, preview: 'https://api.anthropic.com' }, apiKey: { configured: false }, protocol: 'anthropic' },
    ],
    ...
    masters: [{ configured: true, preview: '1234****' }, { configured: true, preview: '9876****' }],
    confirmTimeout: 300,
    guardAction: 'flag',
    ...
    vision: {
      ...
      baseURL: { configured: true, preview: 'https://api.mimo.cn' },
      apiKey: { configured: true, preview: 'mk-****55e0' },
    ...
    miyoushe: { cookie: { configured: true }, defaultGid: 2 },
    pixiv: { enable: true, refreshToken: { configured: true }, imageProxy: 'i.pximg.net', apiProxy: '', maxImages: 5 },
    persona: { dir: 'data/personas' },
    skill: { builtin: true, dir: 'skills', historyCount: 3 },
    search: {
      tavily: { apiKey: { configured: true, preview: 'tvly-****8b2c' } },
      exa: { apiKey: { configured: false } },
      perplexity: { apiKey: { configured: false } },
      brave: { apiKey: { configured: false } },
      searxng: { url: '' },
      ddg: true,
    },
    ...
    mcp: {
      ...
      servers: {
        moegirl: { command: 'npx', args: ['-y', 'moegirl-wiki-mcp'], env: { NODE_ENV: { configured: true } } },
        'filesystem-remote': { type: 'http', url: 'https://mcp.example.com/fs', headers: { Authorization: { configured: true, preview: 'Bearer ****9f' } } },
      },
    },
    ...
    stt: { enable: false, apiBase: 'https://api.openai.com/v1', apiKey: { configured: false }, model: 'whisper-1', language: 'zh' },
    terminal: {
  ...
  const logFiles = [
    { file: '960179589-2854196310-c_1001-20260728_221030.log', label: '群 960179589 · 云汐 · 07-28 22:10', events: [
      ...
      { level: 'info', time: '2026-07-28T22:10:31.020Z', event: 'tool_discovery', traceId: t1, query: '查端口占用 执行命令', category: 'system', hits: [{ name: 'terminal', score: 0.86, category: 'system' }, { name: 'calc', score: 0.31, category: 'compute' }], activated: ['terminal'], alreadyActive: [], activeTotal: 7 },
      { level: 'info', time: '2026-07-28T22:10:33.845Z', event: 'turn', traceId: t1, turn: 1, finish: 'tool_calls', content: null, reasoning: true, toolCalls: [{ name: 'terminal', arguments: '{"cmd":"ss -tlnp | grep 2536"}' }], usage: { prompt_tokens: 3412, prompt_cache_hit_tokens: 2890, prompt_cache_miss_tokens: 522, completion_tokens: 96 }, ms: 2844 },
      { level: 'info', time: '2026-07-28T22:10:34.112Z', event: 'tool', traceId: t1, name: 'terminal', args: { cmd: 'ss -tlnp | grep 2536' }, ok: true, result: 'LISTEN 0 4096 0.0.0.0:2536 … (("redis-server",pid=1145))', ms: 261 },
      { level: 'info', time: '2026-07-28T22:10:36.930Z', event: 'turn', traceId: t1, turn: 2, finish: 'stop', content: '查到了:宿主机 2536 端口被 redis-server 占着,两个方案……', reasoning: false, toolCalls: [], usage: { prompt_tokens: 3621, prompt_cache_hit_tokens: 3104, prompt_cache_miss_tokens: 517, completion_tokens: 215 }, ms: 2810 },
